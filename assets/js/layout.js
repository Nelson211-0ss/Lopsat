(function () {
  /**
   * Set data-base-path from this script’s URL so partials + hero images match the deployed folder
   * (e.g. public_html vs public_html/subdomain/) before any async work runs.
   */
  (function syncBasePathFromScript() {
    var raw = document.documentElement.getAttribute("data-base-path");
    if (raw != null && String(raw).trim() !== "") return;
    var nodes = document.getElementsByTagName("script");
    for (var i = 0; i < nodes.length; i++) {
      var src = nodes[i].src;
      if (!src || src.indexOf("/assets/js/layout.js") === -1) continue;
      try {
        var pathname = new URL(src, typeof location !== "undefined" ? location.href : "").pathname;
        var marker = "/assets/js/layout.js";
        var pos = pathname.lastIndexOf(marker);
        if (pos === -1) continue;
        var seg = pathname.slice(0, pos).replace(/^\/+|\/+$/g, "");
        if (seg) document.documentElement.setAttribute("data-base-path", seg);
      } catch (err) {
        /* ignore */
      }
      break;
